...
<header class="site-header">
  <div class="container">
    ...
    <nav class="nav-main" id="nav-main">
      ...
      ${navItem('Men',    url('pages/men/'),   active === 'men')}
      ${navItem('Women',  url('pages/women/'), active === 'women')}
      ${navItem('Sports', url('pages/sports/'),active === 'sports')}
    </nav>
    <div class="nav-actions">
      <button class="icon-btn" id="search-btn" aria-label="Search"><i class="fas fa-search"></i></button>
      <a href="${url('pages/cart/')}" class="icon-btn" aria-label="Cart">
        <i class="fas fa-shopping-bag"></i>
        <span class="cart-badge" data-count="${cartCount}">${cartCount || ''}</span>
      </a>
      <a href="${url(user ? 'pages/account/profile/' : 'pages/account/login/')}" class="icon-btn" aria-label="Account"><i class="fas fa-user"></i></a>
...
<footer class="site-footer">
  <div class="container">
    <div class="foot-grid">
      ...
      <div class="foot-col">
        <h4>Shop</h4>
        <a href="${url('pages/men/')}">Men</a>
        <a href="${url('pages/women/')}">Women</a>
        <a href="${url('pages/sports/')}">Sports</a>
        <a href="${url('pages/cart/')}">Cart</a>
      </div>
      <div class="foot-col">
        <h4>Account</h4>
        <a href="${url('pages/account/login/')}">Sign In</a>
        <a href="${url('pages/account/signup/')}">Create Account</a>
        <a href="${url('pages/account/profile/')}">My Profile</a>
        <a href="${url('pages/account/checkout/')}">Checkout</a>
      </div>
      <div class="foot-col">
        <h4>Help</h4>
  ...
  const wireHeader = () => {
    ...
    const renderResults = (q) => {
      ...
      results.innerHTML = list.map(p => `
        <a class="search-result" href="${url('pages/product/?id=' + p.id)}">
  ...
  ZEN.initLayout = (active) => {
    ...
    setupReveal();
    // Load AI Chat script
    const aiChat = document.createElement('script');
    aiChat.src = url('assets/js/chatbot/ai-chat.js');
    aiChat.defer = true;
